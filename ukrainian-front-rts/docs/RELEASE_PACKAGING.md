...
The default output is `artifacts/release-package/`. Pass another output path as the first argument when a deploy system needs a staging directory.
...
The builder copies only release runtime inputs: `index.html`, top-level CSS, and the `assets/`, `game/`, `src/`, and `ui/` runtime trees. Authoring sources, tests, task records, scripts, documentation, CI configuration, and diagnostics are not deployed.
...
This verifier is part of the authoritative assembled `verify.sh` plan. Unit tests also mutate package contents and add undeclared files to prove the verifier fails closed.

...
- If backup persistence fails or conflicts with different existing backup contents, the active legacy slot is not rewritten and the load reports `storage-error`.
- Future-version and malformed saves return actionable `unsupported-version` or `corrupt` results and are not rewritten.
...
6. Add unsupported-future, corrupt-data, and destructive-overwrite coverage.

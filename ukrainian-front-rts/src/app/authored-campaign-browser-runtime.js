...
import { initializeAuthoredOperation } from '../systems/authored-operation-runtime.js';
import {
...
export function installAuthoredCampaignBrowserRuntime({
  ...
  const previousGameStart = game.start;
  ...
  game.start = function campaignAwareStart(index = 0, ...args) {
    const prepared = game.pendingAuthoredCampaignOperation
      ?? (game.mission?.authored ? game.authoredCampaignOperation : null);
    if (!prepared) return previousGameStart.call(this, index, ...args);
    const legacyIndex = Math.max(0, Math.min(2, Number.isInteger(index) ? index : 0));
    previousGameStart.call(this, legacyIndex, ...args);
    return initializeAuthoredOperation(this, prepared, { operationIndex: operationIndex(prepared.id) });
  };

...
function requireMixer(mixer) {
  const methods = ['decodeAudioData', 'playBuffer', 'snapshot'];
  if (!mixer || methods.some((method) => typeof mixer[method] !== 'function')) {
    throw new TypeError('Combat SFX runtime requires a compatible audio mixer.');
  }
  return mixer;
}
...
function requestFailure(cue, error) {
  const reason = error instanceof TypeError || error instanceof RangeError
    ? 'invalid-request'
    : 'runtime-error';
  return failure(cue, reason);
}

export async function createCombatSfxRuntime({
  mixer,
  catalogSource,
  ...
  async function preload() {
    ...
    try {
      ...
      for (const bank of catalog.banks) if (!buffers.has(bank.id)) failures.set(bank.id, message);
      return snapshot();
    }

    for (const bank of catalog.banks) {
      if (buffers.has(bank.id)) continue;
      failures.delete(bank.id);
      try {
  ...
  function activeCounts(mixerSnapshot) {
    const counts = {};
    for (const voice of mixerSnapshot.voices ?? []) {
      if (!voice.tag) continue;
      counts[voice.tag] = (counts[voice.tag] ?? 0) + 1;
    }
    return counts;
  }

  function play(cue, request = {}) {
    const asset = catalog.byCue[cue];
    if (!asset) return failure(cue, 'unknown-cue');
    try {
      const mixerSnapshot = audioMixer.snapshot();
      const counts = activeCounts(mixerSnapshot);
      const availableAssetIds = catalog.assets
        .filter((entry) => buffers.has(entry.bankId))
        .map((entry) => entry.id)
        .sort();
      const resolved = resolver.resolve(cue, request, {
        availableAssetIds,
        lastPlayedTicks,
        activeCounts: counts,
      });
      if (!resolved.ok) return resolved;
      const admission = resolver.admit([resolved], {
        availableVoiceSlots: Math.max(0, mixerSnapshot.maxVoices - mixerSnapshot.activeVoiceCount),
        lastPlayedTicks,
        activeCounts: counts,
      });
      if (!admission.accepted.length) {
        const rejected = admission.rejected[0];
        return Object.freeze({
          ok: false,
          cue,
          eventId: resolved.eventId,
          reason: rejected?.reason ?? 'rejected',
          retryAtTick: rejected?.retryAtTick ?? null,
        });
      }
      ...
      const playback = audioMixer.playBuffer({
        buffer,
        bus: resolved.bus,
        volume: resolved.gain,
        tag: resolved.concurrencyKey,
        offset: asset.offsetSeconds,
      ...
      return requestFailure(cue, error);
    }
  }
  ...
  function snapshot() {
    return Object.freeze({
      ...
      loadedBankIds: Object.freeze([...buffers.keys()].sort()),
      loadedAssetIds: Object.freeze(catalog.assets
        .filter((asset) => buffers.has(asset.bankId))
        .map((asset) => asset.id)
        .sort()),
      failures: Object.freeze(Object.fromEntries([...failures.entries()].sort())),
      lastPlayedTicks: Object.freeze({ ...lastPlayedTicks }),
    });
  }
...
export function installCombatSfxDomainAdapter({ events, runtime } = {}) {
  if (!events || typeof events.subscribe !== 'function') {
    throw new TypeError('Combat SFX adapter requires a domain event stream.');
  }
  if (!runtime || typeof runtime.play !== 'function') {
    throw new TypeError('Combat SFX adapter requires a combat SFX runtime.');
  }
  return events.subscribe(DOMAIN_EVENT_TYPES.AUDIO, (event) => {
    const payload = event.payload ?? {};
    if (typeof payload.cue !== 'string' || !payload.cue) return;
    try {
      runtime.play(payload.cue, {
        tick: event.tick,
      ...
      // Audio presentation must never throw through the authoritative event stream.
    }
  });
}

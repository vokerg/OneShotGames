...
function envelope(time, duration, attack = 0.004, releasePower = 2.2) { if (time < 0 || time >= duration) return 0; return Math.min(1, time / Math.max(attack, 1 / COMBAT_SFX_SAMPLE_RATE)) * Math.pow(1 - time / duration, releasePower); }
function pulse(time, at, width, amplitude = 1) { const delta = time - at; if (delta < 0 || delta >= width) return 0; const phase = delta / width; return amplitude * Math.sin(Math.PI * phase) * Math.pow(1 - phase, 1.4); }
function oscillator(time, frequency, phase = 0) { return Math.sin(Math.PI * 2 * frequency * time + phase); }
function chirp(time, startFrequency, endFrequency, duration, phase = 0) { const slope = (endFrequency - startFrequency) / Math.max(duration, 1e-9); return Math.sin(Math.PI * 2 * (startFrequency * time + 0.5 * slope * time * time) + phase); }
...
export function renderCombatSfxRecipe(recipe) {
  ...
  return Object.freeze({ recipe, samples, sampleCount: samples.length, peak: peak > 0 ? 0.92 : 0 });
}
...
export function encodePcm16Wav(samples, { sampleRate = COMBAT_SFX_SAMPLE_RATE } = {}) {
  ...
  view.setUint32(16, 16, true); view.setUint16(20, 1, true); view.setUint16(22, COMBAT_SFX_CHANNELS, true); view.setUint32(24, sampleRate, true);
  view.setUint32(28, sampleRate * 2, true); view.setUint16(32, 2, true); view.setUint16(34, COMBAT_SFX_BITS_PER_SAMPLE, true); writeAscii(bytes, 36, 'data'); view.setUint32(40, dataLength, true);
  for (let index = 0; index < samples.length; index += 1) view.setInt16(44 + index * 2, Math.round(clamp(samples[index]) * 32767), true);
  return bytes;
}

export function buildCombatSfxBanks() {
  ...
  for (const bankId of [...groups.keys()].sort()) {
    ...
    banks.push(Object.freeze({ id: bankId, bytes: encodePcm16Wav(samples), sampleCount: samples.length }));
  }

...
import { enhanceVehicleAtlasSvg } from './vehicle-fidelity-overlay.js';
import {
...
export const UKRAINIAN_VEHICLE_TYPE_ALIASES = Object.freeze({
  'ua.protected-mobility.apc': 'ua.protected-mobility.apc', uaApc: 'ua.protected-mobility.apc',
  'ua.protected-mobility.ifv': 'ua.protected-mobility.ifv', uaIfv: 'ua.protected-mobility.ifv',
  'ua.tank.main-battle': 'ua.tank.main-battle', uaTank: 'ua.tank.main-battle',
  'ua.recovery-vehicle.armored-recovery': 'ua.recovery-vehicle.armored-recovery', uaRecovery: 'ua.recovery-vehicle.armored-recovery',
  'ua.breaching-section.engineering-vehicle': 'ua.breaching-section.engineering-vehicle', uaEngineeringVehicle: 'ua.breaching-section.engineering-vehicle',
});

const ROLE_TO_UNIT_ID = Object.freeze({
  'protected-transport': 'ua.protected-mobility.apc', apc: 'ua.protected-mobility.apc', transport: 'ua.protected-mobility.apc',
  'infantry-fighting-vehicle': 'ua.protected-mobility.ifv', ifv: 'ua.protected-mobility.ifv',
  'main-battle-tank': 'ua.tank.main-battle', tank: 'ua.tank.main-battle',
  'armored-recovery': 'ua.recovery-vehicle.armored-recovery', recovery: 'ua.recovery-vehicle.armored-recovery',
  'combat-engineering-vehicle': 'ua.breaching-section.engineering-vehicle', engineering: 'ua.breaching-section.engineering-vehicle',
});
...
function hasUkrainianIdentity(type, stats) {
  ...
  if (identifiers.some((candidate) => typeof candidate === 'string' && (UKRAINIAN_VEHICLE_TYPE_ALIASES[candidate] || candidate.startsWith('ua.') || /^ua[A-Z]/.test(candidate)))) return true;
  return [stats?.faction, stats?.factionId, stats?.side].some((candidate) => typeof candidate === 'string' && candidate.trim().toLowerCase() === 'ukraine');
}
...
export function ukrainianVehicleDirectionFromAngle(angleRadians) {
  ...
  return UKRAINIAN_VEHICLE_DIRECTIONS[((index % UKRAINIAN_VEHICLE_DIRECTIONS.length) + UKRAINIAN_VEHICLE_DIRECTIONS.length) % UKRAINIAN_VEHICLE_DIRECTIONS.length];
}
export function ukrainianVehicleStateForEntity(entity) {
  ...
  const hp = Number(entity.hp); const maxHp = Number(entity.maxHp);
...
export function ukrainianVehicleAnimationId(unitId, state = 'idle') {
  ...
  return `${resolved}.${UKRAINIAN_VEHICLE_STATES.includes(state) ? state : 'idle'}`;
}
export function ukrainianVehiclePortraitFrameId(unitId) { return `${UKRAINIAN_VEHICLE_TYPE_ALIASES[unitId] ?? unitId}.portrait`; }
export function ukrainianVehicleIconFrameId(unitId) { return `${UKRAINIAN_VEHICLE_TYPE_ALIASES[unitId] ?? unitId}.icon`; }
export function ukrainianVehicleAnimationElapsedMs(entity, state, gameTimeSeconds = 0) {
  if (state === 'attack') { const flash = Math.max(0, Math.min(0.1, Number(entity?.flash) || 0)); return (1 - flash / 0.1) * 310; }
  if (state === 'hit') return Math.max(0, (1 - Math.min(1, Number(entity?.recentHit) || 0)) * 180);
  return Math.max(0, Number(gameTimeSeconds) || 0) * 1000;
}

export async function loadUkrainianVehicleAtlas({ source = UKRAINIAN_VEHICLE_SOURCE_URL, fetchImpl = globalThis.fetch?.bind(globalThis), imageFactory, fallbackRuntime = null } = {}) {
  ...
  const generated = enhanceVehicleAtlasSvg(generateUkrainianVehicleAtlas(await response.json()), { faction: 'ukraine' });
  ...
  return Object.freeze({ ...runtime, catalog: generated.catalogObject, source: generated.source, fidelity: generated.fidelity, generatedSvgBytes: generated.svg.length });
}
...
import {
  ...
  russianVehicleAnimationElapsedMs,
  russianVehicleAnimationId,
  russianVehicleDirectionFromAngle,
  russianVehiclePortraitFrameId,
  russianVehicleStateForEntity,
} from './russian-vehicle-atlas.js';

const INSTALLATION = Symbol.for('fields-of-resolve.russian-vehicle-art-pass');
...
function roleLabel(stats) { return String(stats?.roleId ?? stats?.archetype ?? stats?.role ?? 'armored vehicle').replaceAll('-', ' ').toUpperCase(); }
...
function drawMuzzleFlash(renderer, runtime, resolved, entity, visualState, scale, x, y) {
  ...
  context.save(); context.fillStyle = '#f2d57a'; context.beginPath(); context.arc(point.x, point.y, Math.max(2.5, 4.5 * scale), 0, Math.PI * 2); context.fill(); context.restore();
}

export function installRussianVehicleArtPass(RendererClass, { loadAtlas = loadRussianVehicleAtlas } = {}) {
  if (typeof RendererClass !== 'function' || !RendererClass.prototype) throw new TypeError('Russian vehicle art pass requires a Renderer class.');
  ...
  if (typeof fallbackUnit !== 'function' || typeof fallbackPortrait !== 'function') throw new TypeError('Renderer must expose unit and portrait methods before Russian vehicle atlas installation.');
  ...
  function atlasUnit(entity) {
    ...
    const unitId = stats?.armor === true && !stats?.air ? resolveRussianVehicleAtlasUnitId(entity.type, stats) : null;
    ...
    const visualState = russianVehicleStateForEntity(entity);
    const direction = russianVehicleDirectionFromAngle(entity.angle);
    const scale = Math.max(0.3, zoom * 0.82);
    const animationId = russianVehicleAnimationId(unitId, visualState);
    const elapsedMs = russianVehicleAnimationElapsedMs(entity, visualState, this.g.time);
    const drawX = Math.round(screen.x);
    const drawY = Math.round(screen.y) + 14 * zoom;
    const resolved = state.runtime.drawAnimation(this.x, animationId, { x: drawX, y: drawY, scale, elapsedMs, direction });
  ...
  function atlasPortrait(entity) {
    ...
    context.clearRect(0, 0, 144, 112); context.fillStyle = '#151311'; context.fillRect(0, 0, 144, 112); context.fillStyle = '#29231d';
    for (let y = 0; y < 112; y += 9) for (let x = 0; x < 144; x += 9) if ((x + y) % 27 === 0) context.fillRect(x, y, 9, 9);
    state.runtime.drawFrame(context, russianVehiclePortraitFrameId(unitId), { x: 72, y: 88, scale: 1.45 });
    context.fillStyle = 'rgba(0,0,0,.5)'; context.fillRect(5, 83, 134, 22); context.font = 'bold 10px monospace'; context.fillStyle = '#cdbd9d';
    context.fillText(`${FACTIONS[entity.team]?.short ?? 'RU'} // ${roleLabel(stats)}`, 11, 97); context.strokeStyle = '#745f49'; context.lineWidth = 5; context.strokeRect(2, 2, 140, 108);
    return unitId;
  }

  RendererClass.prototype.unit = atlasUnit;
  RendererClass.prototype.portrait = atlasPortrait;
  RendererClass.prototype.russianVehicleAtlasStatus = function russianVehicleAtlasStatus() { return atlasStatusRecord(state); };
  ...
  const installation = Object.freeze({
    ...
    restore() {
      ...
      delete RendererClass.prototype.russianVehicleAtlasStatus;

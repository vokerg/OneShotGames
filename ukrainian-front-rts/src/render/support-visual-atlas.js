...
function portraitSvg(source,unitId){
  ...
  const prefix=unit?.faction==='ukraine'?'ua':'ru';
...
function manifestFor(source){
  ...
  for(const unit of source.units){
    frames[`${unit.id}.portrait`]={id:`${unit.id}.portrait`};
    frames[`${unit.id}.icon`]={id:`${unit.id}.icon`};
    for(const state of SUPPORT_VISUAL_REQUIRED_STATES){
...
export function resolveSupportVisualUnitId(type,stats=null){
  for(const candidate of [type,stats?.id,stats?.profileId,stats?.rosterNodeId,stats?.visual]){
    ...
    const resolved=resolveUnitId(candidate);
...
export function supportVisualAnimationId(unitId,state='idle'){
  const resolved=resolveUnitId(unitId);
...
export async function loadSupportVisualAtlas({source=SUPPORT_VISUAL_SOURCE_URL,fetchImpl=globalThis.fetch?.bind(globalThis),imageFactory=defaultImageFactory,fallbackRuntime=null}={}){
  try{
    ...
    const sourceObject=await response.json(),errors=validateSupportVisualSource(sourceObject);
    ...
    const cache=new Map(),inflight=new Map(),loadErrors=new Map();
    ...
    const ensureImage=(key,svg)=>{
      ...
      const promise=loadImage(svgDataUrl(svg),imageFactory).then((image)=>{cache.set(key,image);inflight.delete(key);loadErrors.delete(key);return image;}).catch((error)=>{inflight.delete(key);loadErrors.set(key,error);throw error;});
    ...
    function drawAnimation(context,animationId,{x,y,scale=1,alpha=1,elapsedMs=0,direction='n'}={}){
      const parts=animationParts(animationId);
      if(!parts)return Object.freeze({animationId:null,frameId:null,index:0,pending:true});
      const facing=directionId(direction),resolved=resolvedFrameIndex(sourceObject,parts.state,elapsedMs),key=frameKey(parts.unitId,parts.state,facing,resolved.index);
      ...
      drawImage(context,image,{x,y,scale,alpha});
      return Object.freeze({animationId,frameId:`${parts.unitId}.${parts.state}.${facing}.${resolved.index}`,index:resolved.index,pending:false,durationMs:resolved.durationMs,complete:resolved.complete});
    }
    function drawFrame(context,frameId,{x,y,scale=1,alpha=1}={}){
      const suffix='.portrait';
      if(!frameId?.endsWith(suffix))return Object.freeze({frameId,pending:true});
      const unitId=frameId.slice(0,-suffix.length);
      if(!CANONICAL_IDS.has(unitId))return Object.freeze({frameId,pending:true});
      const key=`${unitId}|portrait`,image=cache.get(key);
      if(!image){ensurePortrait(unitId).catch(()=>{});return Object.freeze({frameId,pending:true,loadError:loadErrors.get(key)??null});}
      drawImage(context,image,{x,y,scale,alpha});
      return Object.freeze({frameId,pending:false});
    }
    return Object.freeze({
      manifest,catalog,source:sourceObject,degraded:false,loadError:null,drawAnimation,drawFrame,
      prepare:async(unitIds,{state='idle',direction='e'}={})=>{
        const count=supportVisualReviewFrameCount(sourceObject,state);
        await Promise.all(unitIds.flatMap((unitId)=>Array.from({length:count},(_,index)=>ensureAnimationFrame(unitId,state,direction,index))));
        return Object.freeze({unitCount:unitIds.length,frameCount:unitIds.length*count});
      },

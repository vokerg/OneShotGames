...
export const UI_SKIN_ASSET_DIRECTORY = 'assets/ui/skin';
...
export const UI_SKIN = deepFreeze({
  ...
  assetDirectory: UI_SKIN_ASSET_DIRECTORY,
...
export function uiSkinAssetPath(assetOrId) {
  const entry = assetFor(assetOrId);
  return `${UI_SKIN_ASSET_DIRECTORY}/${entry.id}.svg`;
}

export function buildUiSkinArtifacts(skin = UI_SKIN) {
  validateUiSkin(skin);
  return Object.freeze(skin.assets.map((entry) => Object.freeze({
    id: entry.id,
    path: uiSkinAssetPath(entry),
    content: renderUiSkinAssetSvg(entry),
  })));
}

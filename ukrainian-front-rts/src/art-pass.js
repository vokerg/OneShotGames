...
Renderer.prototype.vehicle=function(q,u,t,z){
 ...
 const w=(tank?40:37)*z,h=(art?48:41)*z,detail=highDetail(z);
 q.save();
 // Tracks establish the widest, darkest silhouette.
 ...
 // Hull uses three large value planes rather than small texture noise.
 ...
 if(tank||art){
  q.fillStyle=p.dark;q.beginPath();q.ellipse(0,-5*z,(tank?13:12)*z,10*z,0,0,Math.PI*2);q.fill();q.strokeStyle=INK;q.lineWidth=Math.max(1,2*z);q.stroke();
  px(q,-8*z,-11*z,12*z,5*z,p.light);
  const barrelTop=(art?53:43)*z,barrelLen=(art?45:35)*z;
  px(q,-3*z,-barrelTop,6*z,barrelLen,p.deep);px(q,-1*z,-barrelTop-2*z,3*z,barrelLen,p.metal);
  if(detail)px(q,5*z,-9*z,5*z,5*z,p.glass);
 }else if(ifv){
...
Renderer.prototype.infantry=function(q,u,t,z){
 ...
 // Exaggerated shoulders, head and equipment preserve class readability at 1x.
 ...
 else if(t.worker){px(q,-11*z,-1*z,22*z,7*z,'#b48142');px(q,-3*z,-27*z,6*z,22*z,p.deep);px(q,-1*z,-28*z,3*z,17*z,p.metal)}
 else if(t.hero){px(q,-11*z,-4*z,22*z,5*z,p.accent);px(q,-3*z,-29*z,6*z,24*z,INK);if(detail)px(q,-8*z,3*z,5*z,7*z,p.light)}

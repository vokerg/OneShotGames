...
test('UFR-160 release UI keeps the action card compact and bounds top-HUD growth', async () => {
  ...
  assert.match(css, /#workerOverview\.workerOverview\s*\{[^}]*grid-template-columns:\s*repeat\(4,/s);
  assert.match(css, /@media \(min-width:\s*1720px\)[\s\S]*grid-template-columns:\s*repeat\(8,/);

...
test('UFR-160 release UI keeps the action card compact, collision-free, and bounds top-HUD growth', async () => {
  ...
  assert.match(css, /@media \(max-width:\s*1280px\) and \(min-width:\s*761px\)[\s\S]*#topbar \.crest\s*\{[^}]*display:\s*none/s);
  assert.match(css, /@media \(max-width:\s*1100px\) and \(min-width:\s*761px\)[\s\S]*#economyHudToggle,\s*\n\s*#techTreeToggle\s*\{[^}]*display:\s*none/s);

...
test('evicts the oldest cache entry when the configured bound is reached', () => {
  ...
  const evictedRequest = service.requestRoute(start, center(grid, 2, 0), {}, { force: true });
...
test('caches bounded failures and reports deterministic search counters', () => {
  ...
  const first = service.requestRoute(center(grid, 0, 0), center(grid, 9, 9), options, { force: true });
  const second = service.requestRoute(center(grid, 0, 0), center(grid, 9, 9), options, { force: true });

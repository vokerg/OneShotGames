...
test('all statically referenced UI and binding keys exist in both runtime catalogs', async () => {
  const [uiSource, adapterSource, viewportSource] = await Promise.all([
    readProjectFile('src/ui.js'),
    readProjectFile('src/localization/runtime-localization.js'),
    readProjectFile('src/ui/viewport-runtime.js'),
  ]);
  ...
  const referencedKeys = new Set([
    ...
    ...[...viewportSource.matchAll(/'((?:app|resources|runtime)\.[A-Za-z0-9.]+)'/g)].map((match) => match[1]),
  ]);
...
test('runtime teardown preserves a pre-existing data-locale owner', async () => {
  const adapterSource = await readProjectFile('src/localization/runtime-localization.js');
  assert.match(adapterSource, /originalLocaleAttribute = .*getAttribute\('data-locale'\)/);
  assert.match(adapterSource, /setAttribute\('data-locale', originalLocaleAttribute\)/);
  assert.match(adapterSource, /removeAttribute\('data-locale'\)/);
});

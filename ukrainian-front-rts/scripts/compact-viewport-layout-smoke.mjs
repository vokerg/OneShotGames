...
try {
  ...
  await waitFor(
    `document.readyState==='complete' && document.querySelector('[data-campaign-operation-id] button:not([disabled])') && window.__fieldsOfResolveViewport?.snapshot && window.__fieldsOfResolveAuthoredCampaign?.snapshot()?.operationCount === 9`,
    'authored operation selector and viewport runtime',
  );
  ...
  await waitFor(
    `document.querySelector('#missionSelect')?.classList.contains('hidden') && document.querySelector('#missionTitle')?.textContent && document.querySelector('#commandPanel') && window.__fieldsOfResolveAuthoredCampaign?.snapshot()?.stage === 'battlefield'`,
    'active authored mission command interface',
  );

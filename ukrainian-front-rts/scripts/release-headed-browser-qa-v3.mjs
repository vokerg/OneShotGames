...
try{
 ...
 await waitFor(`document.querySelector('[data-campaign-operation-id] button:not([disabled])')&&window.__fieldsOfResolveAuthoredCampaign?.snapshot()?.operationCount===9`,'authored operation'); await js(`document.querySelector('[data-campaign-operation-id] button:not([disabled])').click()`); await waitFor(`document.querySelector('[data-campaign-briefing] button.primary')&&window.__fieldsOfResolveAuthoredCampaign?.snapshot()?.stage==='briefing'`,'authored briefing'); await js(`document.querySelector('[data-campaign-briefing] button.primary').click()`); await waitFor(`document.querySelector('#missionSelect')?.classList.contains('hidden')&&window.__fieldsOfResolveAuthoredCampaign?.snapshot()?.stage==='battlefield'`,'mission'); await js(`[...document.querySelectorAll('button')].find(b=>b.textContent?.trim()==='Dismiss all')?.click()`);
 ...
 await keys(ESC); await delay(100); if(await js(`return Boolean(document.querySelector('#pauseMenu')&&!document.querySelector('#pauseMenu').classList.contains('hidden'))`)) await click('#pauseMenuClose'); await waitFor(`document.querySelector('[data-command-id="attack-move"]')?.dataset?.targeting!=='true'`,'attack-move targeting cleanup');
 const perf=await wd('POST','/execute/async',{script:`const done=arguments[arguments.length-1],d=window.__fieldsOfResolvePerformance;if(!d?.reset||!d?.snapshot){done({error:'runtime performance diagnostic unavailable'});return}d.reset();const started=performance.now(),poll=()=>{const s=d.snapshot();if((s?.framesObserved||0)>=90){const sim=s.simulation||{},render=s.render||{},ui=s.ui||{};done({frames:s.framesObserved,mean:(sim.averageMs||0)+(render.averageMs||0)+(ui.averageMs||0),p95:(sim.p95Ms||0)+(render.p95Ms||0)+(ui.p95Ms||0),max:(sim.maximumMs||0)+(render.maximumMs||0)+(ui.maximumMs||0),cadence:{mean:s.frame?.averageMs||0,p95:s.frame?.p95Ms||0,max:s.frame?.maximumMs||0},components:{simulation:sim,render,ui}});return}if(performance.now()-started>14500){done({error:'runtime performance sample timed out',frames:s?.framesObserved||0});return}setTimeout(poll,10)};poll();`,args:[]},15000); assert(!perf.error&&perf.frames>=90&&perf.mean<80&&perf.max<750,`performance ${JSON.stringify(perf)}`); report.surfaces.performance=pass(perf);

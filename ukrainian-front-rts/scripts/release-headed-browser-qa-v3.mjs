...
function capabilities() {
  ...
  if (browser === 'edge') return { browserName:'MicrosoftEdge', 'ms:edgeOptions':{ args:['--disable-search-engine-choice-screen','--disable-dev-shm-usage','--disable-background-timer-throttling','--disable-backgrounding-occluded-windows','--disable-renderer-backgrounding','--disable-features=CalculateNativeWinOcclusion','--disable-frame-rate-limit','--disable-gpu-vsync'] } };
...
try{
 ...
 if(browser==='edge'){ await cdp('Page.bringToFront',{}); await cdp('Emulation.setFocusEmulationEnabled',{enabled:true}); await delay(200); }

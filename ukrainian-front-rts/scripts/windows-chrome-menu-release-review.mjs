...
async function waitFor(expression,label){for(let attempt=0;attempt<120;attempt+=1){try{if(await js(`return Boolean(${expression});`))return}catch{}await delay(100)}throw new Error(`Timed out: ${label}`)}
async function screenshot(name){const data=await wd('GET','/screenshot');await writeFile(join(out,name),Buffer.from(data,'base64'));return name}
...
async function keyChord(key){await wd('POST','/actions',{actions:[{type:'key',id:'kbd',actions:[{type:'keyDown',value:CTRL},{type:'keyDown',value:key},{type:'keyUp',value:key},{type:'keyUp',value:CTRL}]}]});await wd('DELETE','/actions');await delay(250)}
...
function validate(snapshot,label){
 ...
 assert(select.left>=field.left-.5&&select.right<=field.right+.5,`${label}: battlefield select escapes its field`);
 assert(field.left>=body.left-.5&&field.right<=body.right+.5,`${label}: battlefield field escapes content column`);
 assert(select.left>=card.left-.5&&select.right<=card.right+.5,`${label}: battlefield select escapes card`);
 assert(action.left>=card.left-.5&&action.right<=card.right+.5,`${label}: action escapes card`);
 assert(!snapshot.horizontalOverflow,`${label}: page has horizontal overflow`);
 if(body.right<=action.left)assert(select.right<=action.left+.5,`${label}: battlefield select overlaps action column`);
}
...
try{
 ...
 session=created?.sessionId||created?.value?.sessionId;report.capabilities=created?.capabilities||created?.value?.capabilities||{};assert(session,'No Chrome session');
 await wd('POST','/window/rect',{x:0,y:0,width:1440,height:900});await wd('POST','/url',{url:pageUrl});
 ...
 await js(`[...document.querySelectorAll('button')].find(button=>button.textContent?.trim()==='Dismiss all')?.click()`);

 for(const[width,height]of[[1280,720],[1920,1080],[960,760]]){
  await setMetrics(width,height,1);const snapshot=await menuSnapshot();assert(snapshot.viewport.width===width&&snapshot.viewport.height===height,`viewport mismatch ${width}x${height}`);validate(snapshot,`${width}x${height}`);
  await js(`document.querySelector('.skirmishMissionCard')?.scrollIntoView({block:'center'});`);await delay(100);const file=`windows-chrome-menu-${width}x${height}.png`;report.screenshots.push(await screenshot(file));report.viewports.push({width,height,snapshot});
 }

 await clearMetrics();await wd('POST','/window/rect',{x:0,y:0,width:1440,height:900});const beforeZoom=await menuSnapshot();let shortcut='Ctrl+=';await keyChord('=');let afterZoom=await menuSnapshot();
 if(!zoomChanged(beforeZoom,afterZoom)){shortcut='Ctrl++ fallback';await keyChord('+');afterZoom=await menuSnapshot()}
 validate(afterZoom,'browser zoom');assert(zoomChanged(beforeZoom,afterZoom),`Native Chrome zoom shortcut did not change browser scale: before=${JSON.stringify(beforeZoom.viewport)} after=${JSON.stringify(afterZoom.viewport)}`);report.zoom={before:beforeZoom.viewport,after:afterZoom.viewport,shortcut,status:'PASS'};await keyChord('0');

...
- Distance fog, directional shading, red practical-light glow, screen grain, and vignette effects
...
The game remains fully local and dependency-free. `index.html` loads the local `style.css` and `game.js` files; `run.py` remains the one-command launcher.
